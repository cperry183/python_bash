!#/usr/bin/env bash
exec < /home/$(whoami)ssh-key-rotation
set -x
set -e
...
usage() {
  echo "Usage: $0
  -h DOMAIN/IP_ADDRESS [-u SSH_USER] [-b BITS] [-c COMMENT] [-t TYPE] [-f KEY_PATH] [-p PASS] [-k UNIQUE_KEY] "
  exit 1
}
...
removeOldPublicKey() {
  ssh "${SSH_USER}@${DOMAIN}" "sed -i.bak '/${UNIQUE_KEY}/d' ~/.ssh/authorized_key"
}

main() {
  [ -z ${DOMAIN} ] && usage
  # set values if not specified
