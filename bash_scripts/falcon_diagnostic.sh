...
# Copyright (c) CrowdStrike, Inc. 2022-2025. All rights reserved.
...
version_3_11_3='version 3.11.3'   && version_3_11_3_date='March 11, 2025'
version_3_12_0='version 3.12.0'   && version_3_12_0_date='April 1, 2025'
version_3_12_1='version 3.12.1'   && version_3_12_1_date='April 3, 2025'
version_3_13_0='version 3.13.0'   && version_3_13_0_date='April 24, 2025'
version_3_14_0='version 3.14.0'   && version_3_14_0_date='July 14, 2025'
version_current=${version_3_14_0} && version_current_date=${version_3_14_0_date}
...
Changes introduced with ${version_3_11_3} (${version_3_11_3_date}):
    - Capture falcon-flow logs
    - Capture falcon-flow PID stats
    - Fix for detecting the correct PID of falcon-sensor-bpf process

Changes introduced with ${version_3_12_0} (${version_3_12_0_date}):
    - Fix a bug in ${version_3_11_3} that can cause the script to exit with non-zero exit code.

Changes introduced with ${version_3_12_1} (${version_3_12_1_date}):
    - Improve sensor telemetry gathering

Changes introduced with ${version_3_13_0} (${version_3_13_0_date}):
    - Capture falcon-hbfw logs if present

Changes introduced with ${version_3_14_0} (${version_3_14_0_date}):
    - Capture sensor Cloud Id when saved
...
finish() {
    ...
    rm -rf "$diagnostic_subdir" "$um_telemetry_file" "$km_telemetry_file"
...
#
if [ -n "$(/opt/CrowdStrike/falconctl -h 2>&1 | grep loginterval=)" ]; then
...
if [ -z "$unobtrusive" ]; then
    ...
    if [ "$same" -eq 1 ]; then find "$VAR/log" -name "falcon-flow*" -exec ln -s  {} . \;; else find "$VAR/log" -name "falcon-flow*" -exec cp {} . \;; fi
    ...
    if [ "$same" -eq 1 ]; then find "$VAR/log" -name "falcon-hbfw*" -exec ln -s  {} . \;; else find "$VAR/log" -name "falcon-hbfw*" -exec cp {} . \;; fi
...
#
# Identify sensor process PID.
#
# The falcon-sensor-bpf process can fork, leading to multiple PIDs with the command name
# "falcon-sensor-bpf". To find the top-level falcon sensor process, find the process that has
# falcond PID as its PPID:
#
# $ ps -e -o pid,ppid,cmd,comm | grep falcon
#   18114   17980 /opt/CrowdStrike/falcond -f falcond
#   18129   18114 falcon-sensor-bpf           falcon-sensor-b
#   18236   18129 falcon-sensor-bpf           falcon-task
#   18237   18236 falcon-sensor-bpf           falcon-task
#
# $ ps --ppid $(pidof -s falcond) -o pid= | awk '{print $1}'
#   18129
#
sensor_pid=
falcond_pid=$(pidof -s falcond)
if [ -n "${falcond_pid}" ]; then
    sensor_pid="$(ps --ppid "${falcond_pid}" -o pid= | awk '{print $1}')"
fi
...
#
# Add commands for stats specific to a PID. Usage:
#    add_pid_stats PID FILE_NAME_PREFIX
#
add_pid_stats() {
    pid="${1}"
    prefix="${2}"
    add_command "cat /proc/${pid}/maps"         "${prefix}-proc-maps.txt"
    add_command "cat /proc/${pid}/numa_maps"    "${prefix}-proc-numamaps.txt"
    add_command "cat /proc/${pid}/smaps"        "${prefix}-proc-smaps.txt"
    add_command "cat /proc/${pid}/smaps_rollup" "${prefix}-proc-smaps_rollup.txt"
    add_command "cat /proc/${pid}/stack"        "${prefix}-proc-stack.txt"
    add_command "cat /proc/${pid}/status"       "${prefix}-proc-status.txt"
    add_command "lsof -p ${pid}"                "${prefix}-lsof.txt"
    add_command "pmap -x ${pid}"                "${prefix}-pmap-x.txt"
    add_command "pmap -X ${pid}"                "${prefix}-pmap-X.txt"
    add_command "prtstat ${pid}"                "${prefix}-prtstat.txt"
}
...
if [ -z "$unobtrusive" ]; then
    if [ -n "${sensor_pid}" ]; then
        add_pid_stats "${sensor_pid}" "fs"
    fi

    # Add PID stats for the falcon-flow process (if it is running)
    falcon_flow_pid="$(pidof -s falcon-flow)"
    if [ -n "${falcon_flow_pid}" ]; then
        add_pid_stats "${falcon_flow_pid}" "falcon-flow"
    fi
...
add_command 'echo --cloud:              && /opt/CrowdStrike/falconctl -g --cloud && echo'                 falconctl-cloud.txt
...
/opt/CrowdStrike/falconctl -s --logcounters=false >> "$log_file" 2>&1
if [ -n "$telemetry_options" ]; then
    /opt/CrowdStrike/falconctl -d --loginterval --logduration >> "$log_file" 2>&1
fi
...
cgroup_mem_path=$(get_path ${cgroup_mount}${cgroup_path})
find $cgroup_mem_path -maxdepth 1 -perm -u+r -name 'memory.*' -exec cp -av {} . \; >> "$log_file" 2>&1
